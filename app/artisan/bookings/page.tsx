...
export default async function ArtisanBookingsPage() {
  ...
  const bookings = await getBookingsFor(me.id, "artisan");

  return (
    <div>
      <h1 className="text-2xl font-bold">Mes prestations</h1>
      {bookings.length === 0 ? (
        <p className="mt-10 text-center text-muted-foreground">Aucune prestation programmée.</p>
      ) : (
